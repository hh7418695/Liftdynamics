...
# def predition(R, V, params, force_fn, shift_fn, dt, mass, runs=1000, stride=10, R_lead=None, V_lead=None):
#     func = partial(force_fn, R_lead=R_lead, V_lead=V_lead, mass=mass)
#     init, apply = nve(lambda R, V: func(R=R, V=V, params=params), shift_fn, dt)
#     state = init(R, V, mass)
...
def predition(R, V, R_lead, V_lead, params, force_fn, shift_fn, dt, mass, runs=1000, stride=10):
    # print(f"predition: R = {R}, V = {V}, R_lead = {R_lead}, V_lead = {V_lead}\n")
    func = partial(force_fn, mass=mass)
...
# def solve_dynamics(init_state, apply, runs=100, stride=10, R_lead=None, V_lead=None):
#     step = jit(lambda i, state: apply(state, R_lead, V_lead))
#
#     def f(state):
#         y = jax.lax.fori_loop(0, stride, step, state)
#         return y, y
